...
AdElements = Ember.Namespace.create();
AdElements.AdElement = Sortable.Dragable.extend({
  eleData : function() {
    var data = this.get("data"), stateData = this.get("stateData"),
        eleData = stateData.elementsData[data.Name || "LayoutRoot"] || stateData.elementsData.Wrapper;
    return eleData;
  }.property('data', 'stateData'),
  tagName : 'span',
  //classNameBindings : ['eleData.CoreClassName'],
});
AdElements.AdUnit = AdElements.AdElement.extend({
  init : function() {
    this._super();
    var Children = this.get("Children"),
        grpId = this.get("curGrpId"), hierarchy = this.get("hierarchy"),
        data = this.get("data"), stateData = this.get("stateData");
    this.pushObject(AdElements.AdUnitDroppable.create({
      Children : Children,
      groupId : grpId,
      hierarchy : hierarchy,
      data : data,
  ...
  sameLevel : Ember.computed.oneWay('eleData.sameLevel'),
  Children : Ember.computed.oneWay('data.Children'),
  hasElements : Ember.computed.notEmpty('Children'),
  tagName : 'div',
  curGrpId : function() {
    var grpId = this.get("groupId"), stateData = this.get("stateData");
    if(this.get("sameLevel")) return grpId;
    stateData.grpId = grpId + 1;
    return grpId + 1;
  }.property('groupId', 'sameLevel'),

  template : Ember.Handlebars.compile('{{view AdElements.AdUnitDroppable Children=view.Children groupId=view.curGrpId hierarchy=view.hierarchy data=view.data ' +
                                                                        'stateData=view.stateData}}'),
});
AdElements.AdUnitDroppable = Sortable.Droppable.extend({
  init : function() {
    this._super();
    this.loadChildViews();
  },
  ...
  loadChildViews : function() {
    ...
    for(var i = 0; i < Children.length; i++) {
      var childEleData = stateData.elementsData[Children[i].Name] || (stateData.fullCfg.layout.Wrappers && stateData.fullCfg.layout.Wrappers[Children[i].Name]) || {};
      this.pushObject(AdElements[childEleData.view || "AdUnit"].create({
        data : Children[i],
        sortable : this,
        hierarchy : hierarchy,
        groupId : stateData.grpId,
        stateData : stateData,
        tagName : childEleData.TagName || "span",
      }));
    }
  },

  rerenderChildViews : function() {
