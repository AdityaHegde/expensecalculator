  ...
  model : function(params, transtion) {
    /*return new Ember.RSVP.Promise(function(resolve, reject) {
      $.ajax({url : "/page_data", dataType : "json"}).done(function(retdata) {
        ...
        //reject(message);
        resolve(Expense.GlobalData.create({userName : "anonymous"}));
      });
    });*/
    return Expense.GlobalData.create({userName : "anonymous"});
  },
});
...
Expense.OutingRoute = Ember.Route.extend({
  model : function(params, transition) {
    ...
    if(params.outing_id && params.outing_id !== "new" && (!curOuting || curOuting.get("id") != params.outing_id)) {
